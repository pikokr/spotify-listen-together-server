...
export default class Player {
  ...
  getTrackProgress() {
    return this.paused
      ? this.milliseconds
      : this.milliseconds + (Date.now() - this.millisecondsLastUpdate)
  }
  ...
  */
  ...
  changeSong(trackUri: string) {
    if (Player.isTrackListenable(trackUri)) {
      if (this.loadingTrack !== null) clearTimeout(this.loadingTrack)
  ...
  requestUpdateSong(
    ...
    if (info === undefined || info?.isHost) {
      if (this.locked) {
        info?.socket.emit(
          'bottomMessage',
          'Listen together is currently locked!',
          true
        )
      } else {
  ...
  */
  checkTrackLoaded() {
    if (
      this.socketServer
        .getListeners()
        .every((info) => info.trackUri === this.trackUri)
    ) {
  ...
  checkListenerHasAD() {
    let hasAD = this.socketServer
      .getListeners()
      .some((info) => Player.isTrackAd(info.trackUri))
    this.lock(hasAD)
  }
  ...
  checkDesynchronizedListeners() {
    if (this.loadingTrack === null) {
      if (
        this.socketServer
          .getListeners()
          .some((info) => info.trackUri !== this.trackUri)
      ) {
        console.trace()
  ...
  trackLoaded() {
    if (this.loadingTrack) clearTimeout(this.loadingTrack)
  ...
  updateSongInfo(newName?: string, newImage?: string) {
    if (newName != undefined) this.songInfo.name = newName

    if (newImage != undefined)
      if ((newImage.match(/:/g) || []).length === 2)
        this.songInfo.image =
          'https://i.scdn.co/image/' + newImage.split(':')[2]
      else this.songInfo.image = ''
