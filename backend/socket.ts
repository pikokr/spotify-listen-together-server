...
export default class SocketServer {
  ...
  getListeners() {
    return [...this.clientsInfo.values()].filter(
      (info: ClientInfo) => info.loggedIn
    )
  }
  ...
  emitToNonListeners(ev: string, ...args: any) {
    ;[...this.clientsInfo.values()]
      .filter((info: ClientInfo) => !info.loggedIn)
      .forEach((info) => {
  ...
  sendListeners(socket?: Socket) {
    if (!socket) socket = <any>this.io

    socket?.emit(
      'listeners',
      this.getListeners().map((info) => {
        return {
          name: info.name,
          isHost: info.isHost,
  ...
  addEvents(player: Player) {
    ...
    this.io.on('connection', (socket: Socket) => {
      ...
      socket.conn.on('packetCreate', function (packet) {
        if (packet.type === 'ping') lastPing = Date.now()
      })
      ...
      socket.on(
        ...
        (
          ...
          badVersion?: (requirements: string) => void
        ) => {
      ...
      socket.on('requestHost', (password: string) => {
        if (password === config.hostPassword) {
          if (
            [...this.clientsInfo.values()].every(
              (info: ClientInfo) => !info.loggedIn || !info.isHost
            )
          ) {
            this.updateHost(info, true)
          } else {
      ...
      socket.on(
        'changedSong',
        (trackUri: string, songName?: string, songImage?: string) => {
